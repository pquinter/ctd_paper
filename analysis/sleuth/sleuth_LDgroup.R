"""
Long disorder (LD) linear model:
~Replicate + condition + LD + LD*condition
...
so <- sleuth_prep(s2c, ~Replicate + condition + LD + LD*condition,
...
for (b in betas){
    ...
    write.csv(sleuth_results(so, b),
            file=file.path(base_dir, sprintf('sleuth/LDgroup/sleuth_%s.csv', b)),
            quote=FALSE, row.names=FALSE)}

# save sleuth object
saveRDS(so, file=file.path(base_dir, 'sleuth/LDgroup/so.rds'))
